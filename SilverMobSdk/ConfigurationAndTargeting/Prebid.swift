...
public class Prebid: NSObject {
    ...
    //Controls whether to use SilverMobSdk's in-app browser or the Safari App for displaying ad clickthrough content.
    ...
    //If set to true, the output of SilverMobSdk's internal logger is written to a text file. This can be helpful for debugging. Defaults to false.
    ...
    ///
    /// Use this SDK initializer if you're using SilverMobSdk with GMA SDK.
    /// - Parameters:
    ...
    /// The `host` value is obtained from `Prebid.shared.prebidServerHost`.
    ///
    /// Use this SDK initializer if you're using SilverMobSdk without GMA SDK.
    /// - Parameters:
    ///   - completion: returns initialization status and optional error
